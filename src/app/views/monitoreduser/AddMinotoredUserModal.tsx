...
const AddMonitoredUserModal: React.FC<AddMonitoredUserModalProps> = ({ onClose }) => {
    ...
    const sendCode = async () => {
        try {
            setLoading(true);
            await monitoredUserClient.requestAdd(username.value, authType);
            setCodeSended(true);
        } catch { } finally {
            setLoading(false);
        }
    ...
    const confirm = async () => {
        try {
            setLoading(true);
            await monitoredUserClient.confirmAdd(username.value, authType, code.value);
            onClose(true);
        } catch {
            setLoading(false);
        } finally {
    ...
    return <Modal open size="small">
        ...
        <Modal.Content>
            ...
            <Message info>
                ...
                <Message.Content>
                    <List>
                        <List.Item>绑定用户的过程中需要通过向被绑定用户所使用的邮箱或者手机号发送验证码来完成绑定过程。</List.Item>
                        <List.Item>绑定完成后，您可以随时在网站上查看被绑定用户的学习统计情况。如果您绑定了微信账号，那么学习情况每天也会推送给您。</List.Item>
                    </List>
                </Message.Content>
            </Message>
        </Modal.Content>
