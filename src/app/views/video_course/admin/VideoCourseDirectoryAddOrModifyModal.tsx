...
const VideoCourseDirectoryAddOrModifyModal: React.FC<{
    ...
    const doSave = async () => {
        try {
            ...
            if (currentData) {
                await videoRecordPlayClient.updateVideoCourseDirectory(currentData.id, title, JSON.parse(schema) as VideoCourseDirectorySchema, parseInt(order));
            } else {
                await videoRecordPlayClient.addVideoCourseDirectory(title, JSON.parse(schema) as VideoCourseDirectorySchema, parseInt(order));
            }
            onClose(true);
        } catch { } finally {
            setLoading(false);
